...
{
public:
	FrameListener() : Node("frame_listener"), turtle_spawning_service_ready_(false), turtle_spawned_(false)
	{
	...
	void on_timer()
	{
		...
		if (turtle_spawning_service_ready_)
		{
			if (turtle_spawned_)
			{
				...
				try
				{
					transform_msg = tf_buffer->lookupTransform(toFrameRel, fromFrameRel, tf2::TimePointZero);
				}
